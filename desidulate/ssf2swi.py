...
def main():
    ...
    ssf_df = (
        df[df.hashid == args.hashid]
        .drop(["hashid_noclock", "count", "rate", "vol", "hashid", "fltext"], axis=1)
        .reset_index(drop=True)
    )
    atk1, dec1, sus1, rel1, pr_speed, test1_initial = ssf_df[
        ["atk1", "dec1", "sus1", "rel1", "pr_speed", "test1"]
    ].iloc[0]
