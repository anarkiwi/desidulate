...
def main():
    ...
    timer_parser.add_argument(
        "--no-timer", dest="timer", action="store_false", help="do not add timer args"
    )
    ...
    df = pd.read_csv(
        ...
        usecols=["path", "magicID", "sids", "pal", "cia", "song"],
    )
    ...
    for row in df.itertuples():
        ...
        if args.ext:
            ...
            filename = ".".join(
                ("%s/%u/%s-%u" % (filename, row.song, bfilename, row.song), args.ext)
            )
        try:
