...
import unittest
import pandas as pd
from desidulate.sidlib import reg2state, state2ssfs, control_labels
...
class SSFTestCase(unittest.TestCase):
    ...
    def test_adsr(self):
        ...
        self.assertEqual(
            127,
            smf.sid_adsr_to_velocity(0, None, atk1=0, dec1=0, sus1=15, rel1=0, gate1=1),
        )
        self.assertEqual(
            59,
            smf.sid_adsr_to_velocity(0, None, atk1=0, dec1=0, sus1=7, rel1=0, gate1=1),
        )
        self.assertEqual(
            32,
            smf.sid_adsr_to_velocity(
                20e3, None, atk1=7, dec1=0, sus1=0, rel1=0, gate1=1
            ),
        )
        self.assertEqual(
            32,
            smf.sid_adsr_to_velocity(
                20e3, None, atk1=7, dec1=0, sus1=1, rel1=0, gate1=1
            ),
        )
        self.assertEqual(
            8,
            smf.sid_adsr_to_velocity(
                20e3, 20e3, atk1=7, dec1=0, sus1=1, rel1=0, gate1=0
            ),
        )
        self.assertEqual(
            127,
            smf.sid_adsr_to_velocity(0, None, atk1=0, dec1=1, sus1=8, rel1=8, gate1=1),
        )
    ...
    def test_notest_ssf(self):
        df = pd.DataFrame(
            [
                ...
                {"hashid": 1, "count": 1, "clock": 1e5, "gate1": 0},
            ],
            dtype=pd.UInt64Dtype(),
        )
        s = self._df2ssf(df, percussion=True)
        self.assertEqual(s.waveforms, {"t"})
        self.assertEqual(s.midi_pitches, (35,))
        self.assertEqual(s.total_duration, 98280)
        self.assertEqual(
            s.midi_notes, ((0, 35, s.total_duration, MAX_VEL, 60.134765625),)
        )
    ...
    def test_test_ssf(self):
        ...
        s = self._df2ssf(df, percussion=True)
        self.assertEqual(s.waveforms, {"Tt", "t"})
        self.assertEqual(s.midi_pitches, (35,))
        self.assertEqual(s.total_duration, 78624)
        self.assertEqual(
            s.midi_notes, ((20000, 35, s.total_duration, MAX_VEL, 60.134765625),)
        )
